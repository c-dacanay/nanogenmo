...
def get_conflict_sentence(tproperty, a, b, target):
    ...
    CONFLICT_SENTENCES = {
        'hot': {
            ...
            'gt': [
                f'I {adverb} want to go to SeaWorld with you.'
            ], 'lt': [
                f"I {adverb} don't like going to SeaWorld."
            ],
        }, 'extra': {
            'gt': [
                f'I {adverb} want to go hang out with my friends tonight. Do you want to come?',
            ], 'lt': [
                f'I {adverb} wish we spent more time alone together. '
            ],
        }, 'agree': {
            'gt': [
            ...
            'gt': [
                f"You make it {adverb} difficult for me to trust you. "
            ], 'lt': [
                f"Your neuroticism is {adverb} frustrating for me. "
            ]
        }, 'commit': {
            'gt': [
            ...
            'gt': [
                f"I {adverb} want to have more sex. "
            ], 'lt': [
                f"I {adverb} want to have less sex. "
            ]
        }, 'exp': {
            'gt': [
                f"I {adverb} feel like my previous relationships have been more fulfulling than this one. "
            ], 'lt': [
                f"I'm {adverb} jealous of your exes! "
            ]
        }
    }
...
def get_response_sentence(event, a, prop, prop_score):
    ...
    ind = util.clamp(math.floor(prop_score * len(responses)),
                     0, len(responses) - 1)
...
def get(event):
    ...
    character_a = event['protagonist'] if event['protagonist_initiated'] else event['person']
    character_b = event['person'] if event['protagonist_initiated'] else event['protagonist']
    rolls_a = event['protag_rolls'] if event['protagonist_initiated'] else event['person_rolls']
    rolls_b = event['person_rolls'] if event['protagonist_initiated'] else event['protag_rolls']

    dialogue = f"Shortly afterward, {character_a['name']} messaged {character_b['name']} on Discord.\n\n"

    conflict_sentence = get_conflict_sentence(
        event['target_property'], character_a, character_b, event['target'])
    ...
    while team_score + event['handicap'] < event['target']:
        ...
        next_sentence = get_response_sentence(
            event, character_b, char_b_prop, rolls_b[char_b_prop])
        ...
        next_sentence = get_response_sentence(
            event, character_a, char_a_prop, rolls_a[char_a_prop])
    ...
    if team_score > event['target']:
        next_sentence = get_resolution_sentence(
            event, character_a, character_b)
        dialogue += next_sentence
    else:
